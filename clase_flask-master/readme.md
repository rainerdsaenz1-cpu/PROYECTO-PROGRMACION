...
2. Clonar o descargar el proyecto:
   ...
   Code → Download ZIP
4. Crear entorno virtual(Terminal):
   ```
   python -m venv venv
   ...
   ```
5. Instalar las dependencias correspondientes (Terminal):
   ```
   pip install -r requirements.txt
   ```
6. Iniciar MongoDB con Docker(Terminal):
   ```
   docker compose up -d
   ```
7. Inicializar los usuarios demo en la base de datos (primera vez):
   ```(Terminal)
   python init_db.py
